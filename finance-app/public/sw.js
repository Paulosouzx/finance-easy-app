...
// Push: mostra uma notificação com os dados enviados pela Edge Function.
self.addEventListener("push", (event) => {
  let payload = { title: "Finance Easy", body: "Tens uma atualização.", url: "/" };
  try {
    if (event.data) payload = { ...payload, ...event.data.json() };
  } catch {
    // payload não é JSON válido, usa os valores por omissão
  }
...
// Clique na notificação: foca uma janela já aberta na app, ou abre uma nova no destino indicado.
